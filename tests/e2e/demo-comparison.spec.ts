...
test("opens a complete fictional comparison from the landing page", async ({ page }) => {
  ...
  await expect(page.getByText("Lowest Total Cost", { exact: true }).first()).toBeVisible();
  await expect(page.getByRole("heading", { name: "Item-by-Item Comparison" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Print / Export PDF" })).toBeVisible();
  const journey = page.getByRole("navigation", { name: "Comparison progress" });
  await expect(journey.getByRole("listitem")).toHaveText(["✓Upload", "✓Review", "3Compare", "4Export"]);
  await expect(journey.locator('[aria-current="step"]')).toHaveText("3Compare");
...
test("offers all four steps on a small screen and keeps report controls out of print", async ({ page }) => {
  await page.setViewportSize({ width: 360, height: 800 });
  await page.goto("/");
  for (const name of ["Upload", "Review", "Compare", "Export"]) {
    await expect(page.getByRole("heading", { name, exact: true })).toBeVisible();
  }
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true);
  await page.getByRole("link", { name: "View sample comparison" }).click();
  await page.emulateMedia({ media: "print" });
  await expect(page.getByRole("navigation", { name: "Comparison progress" })).toBeHidden();
  await expect(page.getByRole("button", { name: "Print / Export PDF" })).toBeHidden();
  await expect(page.getByRole("heading", { name: "Things to Check" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Item-by-Item Comparison" })).toBeVisible();
});
