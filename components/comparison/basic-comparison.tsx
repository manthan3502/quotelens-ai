  ...
  return (
    <div className="container comparison-page">
      ...
      <header className="comparison-header">
        <div><p className="eyebrow">Quotation Comparison</p><h1>{title}</h1><p className="muted">Compare prices and terms from the details you checked. Lowest cost does not always mean the right vendor for your purchase.</p></div>
        <div className="comparison-actions"><PrintButton />{demo ? <Link className="button secondary" href="/login">Create your comparison</Link> : <Link className="button secondary" href={`/comparisons/${comparisonId}/review`}>Review Quotation Details</Link>}</div>
      </header>

      {comparison.incompatibleCurrencies ? <div className="comparison-alert"><strong>These quotations use different currencies.</strong><span>We compare costs only when quotations have complete details and use the same currency. We do not convert currencies.</span></div> : null}

      <section className="summary-grid" aria-label="Comparison highlights">
        <article className="summary-card"><span>Lowest Total Cost</span><strong>{lowest.length ? lowest.map((item) => item.quote.vendor.name ?? "Unnamed vendor").join(", ") : "Not available"}</strong><small>{lowest.length === 1 ? formatMoney(lowest[0].calculated.computedGrandTotal, lowest[0].quote.currency) : "Requires 2 complete quotes in one currency"}</small></article>
        <article className="summary-card"><span>Fastest Delivery</span><strong>{fastest.length ? fastest.map((item) => item.quote.vendor.name ?? "Unnamed vendor").join(", ") : "Not found"}</strong><small>{fastest[0] ? term(fastest[0].quote.delivery.days, "days", fastest[0].quote.delivery.text) : "Check the quotation"}</small></article>
        <article className="summary-card"><span>Longest Warranty</span><strong>{longest.length ? longest.map((item) => item.quote.vendor.name ?? "Unnamed vendor").join(", ") : "Not found"}</strong><small>{longest[0] ? term(longest[0].quote.warranty.months, "months", longest[0].quote.warranty.text) : "Check the quotation"}</small></article>
        <article className="summary-card"><span>Things to Check</span><strong>{warnings}</strong><small>{warnings ? "See things to check below" : "No issues found in the details you checked"}</small></article>
      </section>
